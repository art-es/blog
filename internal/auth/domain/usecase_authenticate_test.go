...
func TestAuthenticateUsecase(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			expOut: &dto.AuthenticateOut{AccessToken: accessToken},
			expErr: noError,
		},
		{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			u := domain.NewAuthenticateUsecase(userRepository, passwordValidator, accessTokenIssuer)
			out, err := u.Do(ctx, in)
